...
\paragraph{\textsc{Water}.} The same (log-submodular) \flid{} model based on a problem of sensor placement in a water distribution network \citep{krause08} that we also used in the experimental section of last chapter (see \figref{fig:gibbs_exp1}).
...
\paragraph{Results.}
We compare the performance of the Gibbs sampler (\textsc{Gibbs}) against our proposed combined sampler using a proposal mixture $q$ constructed by \algoref{alg:mixture} (\textsc{Combo-I}).
...
In \figref{fig:expising} we show the results on the three log-submodular models described before using mixtures of size $r = 200$.
...
Similar observations can be made with respect to computation time, as shown in \figref{fig:exptime} in the appendix, which measure wall-clock time on the $x$-axis.

...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{\subflen}
    \centering
    \includegraphics[width=\textwidth]{figures/background/flid.pdf}
  \end{subfigure}\\[1.5em]
  \begin{subfigure}[b]{\subflen}
    \centering
    \hspace{-2.65em}\includegraphics[width=1.07\textwidth]{figures/background/flid_dist.pdf}
  \end{subfigure}\\[1em]
  \caption{An example \flid{} model on ground set $V = \{1, 2, 3\}$, the corresponding values of the facility location function $F(S)$, for all $S \subseteq V$, and the resulting distribution $p(S) \propto \exp(F(S))$.}
  \label{fig:flid}
\end{figure}
\end{example}
...
\paragraph{Additively decomposable functions.}
Note that, both the Ising model, and \flid{} use additively decomposable functions, that is, functions that can be written as a sum of simpler supermodular (resp. submodular) functions $F_{j}$,
...
\section{Sampling and Mixing Time}

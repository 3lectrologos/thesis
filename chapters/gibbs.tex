...
Furthermore, if we define $\hrn = \{S \subseteq V \,|\, R \subseteq S\}$, $\hcn = \{S \,|\, C \subseteq S \subseteq V\}$, and $\hkn = 2^V \setminus (\hrn \cup \hcn)$, then the following properties hold.
\begin{align}
...
For submodular and supermodular functions, $\zf$ represents the worst-case amount by which $F$ violates the submodular inequality, and $\zf = 0$ if and only if $F$ is modular (cf. \sectref{sect:bg_submod}).
...
Another quantity that has been used in the past to quantify the deviation of a submodular function from modularity is the curvature \citep{conforti84}, defined as $\kappa_F \defeq 1 - \min_{i \in V} \left(F(i\,|\, V\setminus\{i\}) / F(i)\right)$.
...
If $F$ is submodular or supermodular, this is usually straightforward to accomplish by using one of the standard constant-factor optimization algorithms (see \sectref{sect:bg_submod}) as a preliminary step.
...
Now, for any pair of states $A, B \in \ss$, we define a canonical path
\begin{align*}
...
\begin{align*}
\D_F(i\,|\, S) \defeq F(S \cup \{i\}) - F(S \setminus \{i\}),
\end{align*}
and define the following quantity,
\begin{align*}
  \gf &\defeq \max_{\substack{S \subseteq V\\r \in V}} \sum_{\substack{i \in V}} \tanh\left(\frac{\beta}{2} \Big|\D_F(i\,|\, S) - \D_F(i\,|\, S \cup \{r\})\Big| \right),
\end{align*}
...
\begin{theorem} \label{thm:fast}
  ...
  \begin{align*}
  	\tme \leq \frac{1}{1 - \gf}n\left(\log n + \log \frac{1}{\epsilon}\right).
  \end{align*}
  If $F$ is additionally submodular or supermodular, and is decomposed according to \defref{def:decomp}, then
  \begin{align*}
  	\tme \leq \frac{1}{1 - \gsf}n\left(\log n + \log \frac{1}{\epsilon}\right).
  \end{align*}
\end{theorem}
...
Our proof of \theoremref{thm:fast} is based on the coupling technique \citep{aldous83}; more specifically, we use the \emph{path coupling} method \citep{bubley97,levin08,jerrum03}.
...
More concretely, let us denote by $d : \ss \times \ss \to \mathbb{R}$ the path metric on state space $\ss$; that is, for any $x, y \in \ss$, $d(x, y)$ is the minimum length of any path from $x$ to $y$ in the state space graph.
...
\begin{align*}
  \E[d(X_{t+1}, Y_{t+1}) \,|\, X_t = x, Y_t = y] \leq e^{-\alpha}d(x, y).
\end{align*}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \left|\frac{\exp(\beta \D_F(i\,|\, S))}{1 + \exp(\beta \D_F(i\,|\, S))} - \frac{\exp(\beta \D_F(i\,|\, R))}{1 + \exp(\beta \D_F(i\,|\, R))}\right|\\[0.8em]
    &= \left|\frac{\exp(\beta \D_F(i\,|\, S)) - \exp(\beta \D_F(i\,|\, R))}{(1 + \exp(\beta \D_F(i\,|\, S)))(1 + \exp(\beta \D_F(i\,|\, R)))}\right|\\[0.8em]
    &\leq \left|\frac{\exp(\beta \D_F(i\,|\, S)) - \exp(\beta \D_F(i\,|\, R))}{\exp(\beta \D_F(i\,|\, S)) + \exp(\beta \D_F(i\,|\, R))}\right|\\[0.8em]
    &= \left|\frac{\exp(\beta (\D_F(i\,|\, S) - \D_F(i\,|\, R))) - 1}{\exp(\beta (\D_F(i\,|\, S) - \D_F(i\,|\, R))) + 1}\right|\\[0.8em]
    &= \tanh \left(\frac{\beta}{2}\big|(\D_F(i\,|\, S) - \D_F(i\,|\, R)) \big|\right).
  \end{align*}
...
\begin{lemma}
  If $F$ is submodular or supermodular, and decomposed according to \defref{def:decomp}, then
  \begin{align*}
    \gf = \gsf.
  \end{align*}
\end{lemma}
...
\begin{proof}
  ...
  \begin{align*}
  	&\sum_{i \neq r} \tanh \left(\frac{\beta}{2}\big|(\D_f(i\,|\, S) - \D_f(i\,|\, R)) \big|\right)\\
    &\leq \sum_{i \neq r} \frac{\beta}{2}\big|(\D_f(i\,|\, S) - \D_f(i\,|\, R)) \big| \tag*{by $\tanh(x) \leq x$, for all $x \geq 0$}\\
    &\leq \sum_{i \neq r} \frac{\beta}{2}(\D_f(i\,|\, S) - \D_f(i\,|\, R)) \tag*{by submodularity of $f$}\\
...
We obtain estimates for $\log(Z)$ via a Gibbs-based reverse importance sampling procedure (see \sectref{sect:sampling}), using $200$, $1000$, and $5000$ samples.
...
\figref{fig:gibbs_zest} shows the estimation errors with respect to this approximately true value; error bars depict two standard errors.
...
The marginals we compute are of the form $p(i \in S \,|\, C \subseteq S \subseteq D)$, for all $i \in V$.
